...
// regex: la entrega en [a-zA-Z0-9 ,.]+</p>
template delivery_addressRegex(msg_bytes) {
	...
	signal states_tmp[num_bytes+1][20];
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		lt[1][i].in[1] <== 90;
		and[14][i] = AND();
		and[14][i].a <== lt[0][i].out;
		and[14][i].b <== lt[1][i].out;
		lt[2][i] = LessEqThan(8);
		lt[2][i].in[0] <== 97;
		lt[2][i].in[1] <== in[i];
		lt[3][i] = LessEqThan(8);
		...
		lt[3][i].in[1] <== 122;
		and[15][i] = AND();
		and[15][i].a <== lt[2][i].out;
		and[15][i].b <== lt[3][i].out;
		eq[8][i] = IsEqual();
		eq[8][i].in[0] <== in[i];
		eq[8][i].in[1] <== 44;
		eq[9][i] = IsEqual();
		...
		multi_or[0][i].in[0] <== and[14][i].out;
		multi_or[0][i].in[1] <== and[15][i].out;
		...
		multi_or[0][i].in[12] <== eq[17][i].out;
		multi_or[0][i].in[13] <== eq[18][i].out;
		multi_or[0][i].in[14] <== eq[19][i].out;
		...
		from_zero_enabled[i] <== MultiNOR(19)([states_tmp[i+1][1], states[i+1][2], states[i+1][3], states[i+1][4], states[i+1][5], states[i+1][6], states[i+1][7], states[i+1][8], states[i+1][9], states[i+1][10], states[i+1][11], states[i+1][12], states[i+1][13], states[i+1][14], states[i+1][15], states[i+1][16], states[i+1][17], states[i+1][18], states[i+1][19]]);
	...
	for (var i = 0; i <= num_bytes; i++) {
		is_accepted.in[i] <== states[i][19];
	}
	out <== is_accepted.out;
	signal is_consecutive[msg_bytes+1][3];
	is_consecutive[msg_bytes][2] <== 0;
	for (var i = 0; i < msg_bytes; i++) {
		is_consecutive[msg_bytes-1-i][0] <== states[num_bytes-i][19] * (1 - is_consecutive[msg_bytes-i][2]) + is_consecutive[msg_bytes-i][2];
		is_consecutive[msg_bytes-1-i][1] <== state_changed[msg_bytes-i].out * is_consecutive[msg_bytes-1-i][0];
		is_consecutive[msg_bytes-1-i][2] <== ORAnd()([(1 - from_zero_enabled[msg_bytes-i+1]), states[num_bytes-i][19], is_consecutive[msg_bytes-1-i][1]]);
	}
