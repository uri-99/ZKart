...
// regex: important">[a-zA-Z0-9 .,]+</h1>
template product_nameRegex(msg_bytes) {
	...
	for (var i = 0; i < num_bytes; i++) {
		...
		multi_or[0][i].in[11] <== eq[19][i].out;
		multi_or[0][i].in[12] <== eq[20][i].out;
		multi_or[0][i].in[13] <== eq[21][i].out;
		multi_or[0][i].in[14] <== eq[22][i].out;
		...
		states[i+1][12] <== multi_or[1][i].out;
		eq[23][i] = IsEqual();
		eq[23][i].in[0] <== in[i];
		eq[23][i].in[1] <== 60;
		and[15][i] = AND();
	...
	// substrings calculated: [{(11, 12), (12, 12)}]
	signal prev_states0[2][msg_bytes];
	signal is_substr0[msg_bytes];
	signal is_reveal0[msg_bytes];
	signal output reveal0[msg_bytes];
	for (var i = 0; i < msg_bytes; i++) {
		 // the 0-th substring transitions: [(11, 12), (12, 12)]
		prev_states0[0][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][11];
		prev_states0[1][i] <== (1 - from_zero_enabled[i+1]) * states[i+1][12];
		is_substr0[i] <== MultiOR(2)([prev_states0[0][i] * states[i+2][12], prev_states0[1][i] * states[i+2][12]]);
